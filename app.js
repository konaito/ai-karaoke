import { SONG as INITIAL_SONG, CATALOG, SONG_ID as INITIAL_SONG_ID } from "./song-data.js?v=28";
import { createScoringController } from "./scoring-ui.js?v=28";
import { mixScoringGuide } from "./scoring.js?v=28";
...
async function loadAlignment() {
  const song = SONG;
  const response = await fetch(`${SONG.alignmentSource}?v=28`);
...
async function changeSong(id, autoplay = false) {
  ...
  try {
    const response = await fetch(`${entry.data}?v=28`);
